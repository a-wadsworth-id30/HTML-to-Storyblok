...
The desktop app is intentionally a thin layer over the existing CLI. It does not duplicate planner, validator, generator, Storyblok, route, rollback, or reporting logic. The default screen now starts with quick setup cards: `Test Storyblok`, `Add To Existing Site`, and `Check Previous Work`. These cards select the right workflow, highlight the fields needed for that setup, and keep the existing-site field optional for Storyblok-only testing. The guided workflow still uses plain-language paths such as `Test In Storyblok Only`, `Add To An Existing Site`, `Check Previous Work`, and `Prepare Handoff`. Selecting any step shows contextual help covering what the user needs, what will happen, safety, review files, and recovery notes. Advanced setup fields and direct command buttons remain available in collapsed sections for experienced users and run whitelisted CLI actions such as onboarding, doctor, template inspection, repository inspection, Storyblok inspection, plan, validate, dry run, real apply, route handoff, Storyblok validation, report generation, evidence index, handoff pack, and rollback preview.
...
- Packaged builds include the CLI, desktop shell, source services, templates, and ID30 logo. `asar` is disabled until bundled child CLI execution is verified against an archived app package.
...
- PM, design, QA, account, and content team members can use the desktop app to run safe checks, inspect evidence, and follow the same import runbook without memorising commands. The normal desktop path is now a couple-click setup: choose Storyblok-only testing or a full existing-site import, fill the highlighted fields, then follow the ordered steps.
