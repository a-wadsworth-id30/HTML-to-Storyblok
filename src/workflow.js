...
import { createDraftStories, createStoryblokAssetFolders, createStoryblokComponentGroups, createStoryblokComponents, createStoryblokInternalTags, createStoryblokPresets, getStoryblokConfig, preflightStoryblokIntegration, uploadStoryblokAssets, validateStoryblokDraftContent } from './storyblok.js';
...
export async function applyManifest(manifest, args = {}, workDir, { onProgress = null } = {}) {
  ...
  await progress({ label: 'Checking Storyblok Access', current: 0, total: 12 });
  ...
  await progress({ label: 'Creating Frontend', current: 1, total: 12 });
  await recordApplyStep(workDir, steps, 'apply-step-01-duplicate.json', await duplicateAll(manifest, { repoPath, dryRun, env }));
  await progress({ label: 'Creating Frontend', current: 2, total: 12 });
  ...
  await progress({ label: 'Validating Local Output', current: 3, total: 12 });
  ...
  await progress({ label: 'Creating Storyblok Component Folders', current: 4, total: 12 });
  ...
  await progress({ label: 'Creating Storyblok Internal Tags', current: 5, total: 12 });
  ...
  await progress({ label: 'Creating Storyblok Components', current: 6, total: 12 });
  ...
  await progress({ label: 'Creating Storyblok Asset Folders', current: 7, total: 12 });
  ...
  await progress({ label: 'Uploading Assets', current: 8, total: 12 });
  ...
  await progress({ label: 'Creating Storyblok Presets', current: 9, total: 12 });
  ...
  await progress({ label: 'Creating Draft Stories', current: 10, total: 12 });
  ...
  await progress({ label: 'Validating Storyblok Content', current: 11, total: 12 });
  ...
  await progress({ label: 'Done', current: 12, total: 12 });
  const result = {
