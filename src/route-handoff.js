...
export async function wireRepositoryRoutes(manifest, {
  ...
  const routeCollisionAnalysis = await analyzeRouteCollisions(manifest, {
    repoPath: root,
    adapterPlan: plan,
    route
  });
  const collisionBySlug = new Map(ensureArray(routeCollisionAnalysis.routes).map((entry) => [entry.slug, entry]));
  const routes = await Promise.all(selectedRoutes.map((entry) => planRouteHandoff(root, plan, entry, {
    dryRun,
    collision: collisionBySlug.get(entry.slug)
  })));
  const blocked = routes.filter((entry) => entry.status === 'blocked');
  if (blocked.length > 0) {
    return routeHandoffResult({
      dryRun,
      routes,
      status: 'blocked',
      reason: 'One or more host route targets are unavailable. No files were written.',
  ...
  const status = routes.some((entry) => entry.status === 'created' || entry.status === 'would_create')
    ? 'passed'
    : 'skipped';
  return routeHandoffResult({ dryRun, routes, status, routeCollisionAnalysis });
}

export function renderRouteHandoffReport(result) {
  const routes = ensureArray(result.routes);
  const manualRoutes = routes.filter((route) => route.manual_handoff);
  return `# Repository Route Handoff Report
...
- Policy: ${result.policy || 'additive-only-route-handoff'}
- Dry run: ${result.dry_run ? 'yes' : 'no'}
- Host routes modified: ${result.host_routes_modified ? 'yes' : 'no'}
- Host route files created: ${result.host_route_files_created || 0}
- Routes checked: ${routes.length}
- Created: ${result.summary?.created || 0}
- Would create: ${result.summary?.would_create || 0}
- Blocked: ${result.summary?.blocked || 0}
- Manual handoff routes: ${manualRoutes.length}
${result.route_collision_analysis ? `- Route collision analysis: ${result.route_collision_analysis.status}
- Route collision blockers: ${result.route_collision_analysis.summary?.blocked || 0}
- Route collision warnings: ${result.route_collision_analysis.summary?.warnings || 0}
` : ''}${result.reason ? `- Reason: ${result.reason}\n` : ''}
## Route Evidence

${routes.length ? routes.map(renderRouteEvidence).join('\n\n') : '- No routes were checked.'}

## Route Collision Analysis

${result.route_collision_analysis ? renderRouteCollisionReport(result.route_collision_analysis) : '- Route collision analysis was not recorded.'}

## Manual Handoff

${manualRoutes.length ? renderManualHandoffEvidence(result, manualRoutes) : '- No manual route handoff required for this run.'}

## Required Checks

- Review every generated route proposal before exposing it through the host app.
- Run \`html-to-storyblok validate --manifest <manifest> --repo <repo>\`.
- Run the target repository install, lint, typecheck where available, and build.
- Run browser smoke tests for every imported route.
- Confirm deployed routes expose \`data-hts-storyblok-source="storyblok-draft"\` when live Storyblok draft preview is expected.
`;
}

async function planRouteHandoff(root, plan, route, { dryRun, collision = null }) {
  ...
  const base = {
    ...
    route_collision: collision ? summarizeRouteCollision(collision) : null
  };
  ...
  if (collision?.status === 'blocked') {
    return {
      ...base,
      status: 'blocked',
      reason: `Route collision analysis blocked ${route.suggested_site_path || route.slug}: ${collision.blockers.join(', ') || 'blocked'}.`
    };
  }
...
function routeHandoffResult({ dryRun, routes, status, reason = null, routeCollisionAnalysis = null }) {
...
function summarizeRouteCollision(collision) {
  return {
    status: collision.status,
    blockers: ensureArray(collision.blockers),
    warnings: ensureArray(collision.warnings),
    checks: ensureArray(collision.checks).map((check) => ({
      name: check.name,
      status: check.status,
      message: check.message,
      details: ensureArray(check.details).slice(0, 5)
    }))
  };
}
...
function manualRouteHandoff(plan, route, proposalFile) {
  ...
  return {
    ...
    integration_options: manualRouteIntegrationOptions(framework, { importTarget, sitePath, storyblokSlug }),
...
function manualRouteIntegrationOptions(framework, { importTarget, sitePath, storyblokSlug }) {
  if (framework === 'react') {
    return [
      `React Router: add a route for ${sitePath} that renders the generated proposal from ${importTarget}.`,
      `File-based React routing: create the host page file for ${sitePath} and import the generated proposal component.`,
      `Custom app shell: mount the proposal inside the existing route switch and pass Storyblok draft content as story/blok props.`,
      `Server/runtime data: fetch Storyblok draft ${storyblokSlug} only through a safe server or runtime boundary; never ship Management API credentials.`
    ];
  }
  if (framework === 'vue') {
    return [
      `Vue Router: add a route record for ${sitePath} whose component imports ${importTarget}.`,
      `File-based Vue routing: create the host page file for ${sitePath} and import the generated proposal component.`,
      `Custom app shell: mount the proposal inside the existing view registry and pass Storyblok draft content as story/blok props.`,
      `Server/runtime data: fetch Storyblok draft ${storyblokSlug} only through a safe server or runtime boundary; never ship Management API credentials.`
    ];
  }
  if (framework === 'static') {
    return [
      `Create or update the static route file for ${sitePath} only after reviewing ${importTarget}.`,
      `Use the generated HTML renderer from reviewed host code and keep imported assets under the integration namespace.`,
      `Confirm deployment routing, rewrites, and canonical URLs manually for ${sitePath}.`
    ];
  }
  return [
    `Review ${importTarget} and wire ${sitePath} using the host framework's routing convention.`,
    `Map the route to Storyblok draft ${storyblokSlug} without exposing Management API credentials.`
  ];
}
...
- Status: ${route.status || 'recorded'}
- Storyblok draft: ${route.storyblok_slug || 'not supplied'}
- Source page: ${route.source_page || 'not supplied'}
- Route proposal: ${route.route_proposal_file || 'not supplied'}
- Suggested host route: ${route.host_route_file || ensureArray(route.suggested_host_files).join(', ') || 'manual review'}
- Content source: ${route.content_source || 'not supplied'}
- Import path: ${route.import_path || 'not applicable'}
- Route collision: ${route.route_collision?.status || 'not recorded'}
- SEO title: ${seo.title || 'not supplied'}
- SEO description: ${seo.description || 'not supplied'}
${route.reason ? `- Reason: ${route.reason}\n` : ''}${route.manual_handoff ? `- Manual handoff: ${route.manual_handoff.reason}` : ''}`;
}

function renderManualHandoffEvidence(result, manualRoutes) {
  const summary = result.manual_handoff;
  const summaryRows = summary
    ? [
      `- Frameworks: ${ensureArray(summary.frameworks).join(', ') || 'not supplied'}`,
      `- Routes: ${summary.routes || manualRoutes.length}`,
      `- Policy: ${summary.policy || 'manual-review-required'}`,
      `- Summary: ${summary.summary || 'Manual route registration is required.'}`
    ].join('\n')
    : '';
  const routeRows = manualRoutes.map((route) => `### ${route.suggested_site_path || route.slug}

${ensureArray(route.manual_handoff.integration_options).map((item) => `- ${item}`).join('\n')}

Next steps:
${ensureArray(route.manual_handoff.steps).map((step) => `- ${step}`).join('\n')}`).join('\n\n');
  return `${summaryRows}
